...
ema_Encoder = POINT_encoder(num_classes=2, num_instances=args.ni,h=args.h,N=args.N)
ema_Relation = POINT_relation(num_classes=2, num_instances=args.ni,h=args.h,N=args.N)
ema_Classifier = classifier()

for param in ema_Encoder.parameters():
    param.detach_()
for param in ema_Relation.parameters():
    param.detach_()
...
def main(epoch, val_mAP,bestResult,test_mAP):
    ...
    for batch_idx in tqdm(range(args.training_step)):
        ...
        for i in range(2):
            ...
            unsrc2,unface2, unfaceCont2,uncoor2 = unsrcs[1].cuda(),unfaces[1].cuda(), unfaceConts[1].cuda(),uncoors[1].cuda()
            ...
            with torch.no_grad():
                ...
                unlabeled_gfeat, unlabeled_pfeat = ema_Encoder(unsrc1,unface1, unfaceCont1,uncoor1)
                unlabeled_Ifeat = ema_Relation(unlabeled_gfeat, unlabeled_pfeat)
                ema_pseudo = ema_Classifier(unlabeled_Ifeat)
                ...
                #select the top args.ni
                select_unsrc1,select_unface1,select_unfaceCont1,select_uncoor1,select_IS=[],[],[],[],[]
                select_unsrc2,select_unface2,select_unfaceCont2,select_uncoor2=[],[],[],[]
                ...
                select_unsrc2.append(unsrc2[vip])
                select_unface2.append(unface2[vip])
                select_unfaceCont2.append(unfaceCont2[vip])
                select_uncoor2.append(uncoor2[vip])
                ...
                select_unsrc2.append(unsrc2[nonvips])
                select_unface2.append(unface2[nonvips])
                select_unfaceCont2.append(unfaceCont2[nonvips])
                select_uncoor2.append(uncoor2[nonvips])
                ...
                unsrc2 = torch.cat(select_unsrc2,0)
                unface2 = torch.cat(select_unface2,0)
                unfaceCont2 = torch.cat(select_unfaceCont2,0)
                uncoor2 = torch.cat(select_uncoor2,0)
                subtargets = [0 for i in range(args.ni)]
                subtargets[0]=1
            unlabeled_gfeat, unlabeled_pfeat = Encoder(unsrc2,unface2, unfaceCont2,uncoor2)
            ...
            prelogits = F.softmax(unlabeled_logit)
            if(torch.max(prelogits[:,1]).item()>0.5):
                vip_num+=1
            unlabeled_target.append(subtargets)

        unlabeled_logits = torch.cat(unlabeled_logits)
        ...
        with torch.no_grad():
            ...
            unlabeled_target = mask.scatter(1,pseudo_label,1)
        consistency_loss_u = MSEloss(unlabeled_logits, unlabeled_target,ISWs,EWs)
        ...
        loss = celoss+gamma*consistency_loss_u

        loss.backward()
        optimizer.step()
        update_ema_variables(Encoder, ema_Encoder, args.ema_decay, global_step)
        update_ema_variables(Relation, ema_Relation, args.ema_decay, global_step)
        update_ema_variables(Classifier, ema_Classifier, args.ema_decay, global_step)

        # total_mem+=enloss.item()
        total_loss+=celoss.item()
        total_unloss+=consistency_loss_u.item()
    ...
    train_acc = accuracy/num
    print('Epoch: {} Time: {} label_Loss: {:.6f} consistency_loss: {:.6f} Val mAP: {:.4f} Test mAP {:.4f} Train Acc: {:.4f}'.format(
                    epoch,time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time())),total_loss/len(label_loader),total_unloss/len(label_loader),val_mAP,test_mAP,train_acc))
    ...
    recorder.update('label loss', total_loss/len(label_loader), epoch)
    recorder.update('unlabel loss', total_unloss/len(label_loader), epoch)
    recorder.update('train acc', accuracy/num, epoch)
    recorder.update('unlabel entropy acc', total_entropy_loss/len(label_loader), epoch)
    if epoch%args.interval==0 and epoch!=0:
        ...
        with torch.no_grad():
            ...
            for val_batch_idx, (val_src,val_face, val_faceCont,val_coor, val_label, val_ImgId) in enumerate(tqdm(val_loader)):
                ...
                gfeat, pfeat = ema_Encoder(val_src,val_face,val_faceCont,val_coor)
            ...
            for idx,ids in enumerate(tqdm(imgIDs)):
                ...
                feats = ema_Relation(gfeats,pfeats)
                logits = ema_Classifier(feats)
            ...
            state = {'encoder':ema_Encoder.state_dict(),'relation':ema_Relation.state_dict(), 'classifier': ema_Classifier.state_dict(), 'args':args,'seed':seed}
            ...
            for test_batch_idx, (test_src,test_face, test_faceCont,test_coor, test_label, test_ImgId) in enumerate(tqdm(test_loader)):
                ...
                gfeat, pfeat = ema_Encoder(test_src,test_face,test_faceCont,test_coor)
                gfeat = gfeat.data.cpu()
                pfeat = pfeat.data.cpu()
