...
def main(epoch, val_mAP,bestResult,test_mAP):
    ...
    for batch_idx in tqdm(range(args.training_step)):
        ...
        for i in range(2):
            ...
            with torch.no_grad():
                ...
                ema_pseudo = LP(labeled_Ifeat,target_var,unlabeled_Ifeat)
                ...
                vip = np.array([torch.argmax(ISW).item()])
        ...
        mseloss = MSEloss(unlabeled_logits,unlabeled_target,ISWs,EWs)
    ...
    recorder.update('important score', IS, epoch)      
