...
class RelationNetRandomFaceSampler(Sampler):
    def __init__(self, data_source, num_instances=1):
        ...
        self.Img_dic = defaultdict(list)
        self.Imp_dic = defaultdict(list)
        self.total_dic=defaultdict(list)
        for index, (src,Face, FaceCont,Coor,label, ImgId) in enumerate(data_source):
            # for ind, frame in enumerate(frames):
            if label == 0:
                self.Img_dic[ImgId].append(index)  #[Img_dic[ImgId] is a list which contains the sample index of ImagId]
            if label == 1:
                self.Imp_dic[ImgId].append(index)
            self.total_dic[ImgId].append(index)
        self.ImgId = list(self.Img_dic.keys())
        self.num_samples = len(self.ImgId)
        # self.sample()
...
class SemiNetUnlabelDatasetSampler(Sampler):
    def __init__(self, data_source, num_instances=1):
        self.data_source = data_source
        self.num_instances = num_instances
        self.Img_dic = defaultdict(list)
        ...
        for index, (src,Face, FaceCont,Coor,label, ImgId) in enumerate(data_source):
            self.total_dic[ImgId].append(index)
        self.ImgId = list(self.total_dic.keys())
        self.num_samples = len(self.ImgId)
        # self.sample()
    ...
    def __iter__(self):
        ...
        for i in indices:
            ImgId = self.ImgId[i]
            peoples = self.total_dic[ImgId]
            if len(peoples) >= self.num_instances:
                peoples = np.random.choice(peoples, size=self.num_instances, replace=False)
            else:
                peoples = np.random.choice(peoples, size=self.num_instances, replace=True)
            ret.extend(peoples)
        self.ret = ret
        return iter(ret)
