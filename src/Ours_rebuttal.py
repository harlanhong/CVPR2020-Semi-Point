...
pretrain_Encoder = POINT_encoder(num_classes=2, num_instances=args.ni,h=args.h,N=args.N)
pretrain_Relation = POINT_relation(num_classes=2, num_instances=args.ni,h=args.h,N=args.N)
pretrain_Classifier = classifier()
...
if args.cuda:
    ...
    pretrain_Encoder = pretrain_Encoder.cuda()
    pretrain_Relation = pretrain_Relation.cuda()
    pretrain_Classifier = pretrain_Classifier.cuda()
    # model_teacher.cuda()
global global_step
global_step=0
pretrain_Encoder.eval()
pretrain_Relation.eval()
pretrain_Classifier.eval()

CEloss = nn.CrossEntropyLoss().cuda()
MSEloss = getattr(loss, args.consistency_loss)().cuda()

if True:
    dict_pretrain = torch.load('../models/best_EMS_baseline_5074.pkl')
    dict_model = pretrain_Encoder.state_dict().copy()
    ...
    pretrain_Encoder.load_state_dict(dict_model)

    dict_model = pretrain_Relation.state_dict().copy()
    pretrained_dict = {k: v for k, v in dict_pretrain.items() if k in dict_model}#filter out unnecessary keys 
    dict_model.update(pretrained_dict)
    pretrain_Relation.load_state_dict(dict_model)

    dict_model = pretrain_Classifier.state_dict().copy()
    pretrained_dict = {k: v for k, v in dict_pretrain.items() if k in dict_model}#filter out unnecessary keys 
    dict_model.update(pretrained_dict)
    pretrain_Classifier.load_state_dict(dict_model)
...
def main(epoch, val_mAP,bestResult,test_mAP):
    ...
    for batch_idx in tqdm(range(args.training_step)):
        ...
        unlabeled_target = []
        unlabel_srcs, unlabel_faces,unlabel_faceconts,unlabel_coors,unlabel_labels = [],[],[],[],[]
        for i in range(2):
            ...
            with torch.no_grad():
                unlabeled_gfeat, unlabeled_pfeat = pretrain_Encoder(unsrc1,unface1, unfaceCont1,uncoor1)
                unlabeled_Ifeat = pretrain_Relation(unlabeled_gfeat, unlabeled_pfeat)
                unlabeled_logits = pretrain_Classifier(unlabeled_Ifeat)

                vipMask = unlabeled_logits[:,1] == torch.max(unlabeled_logits[:,1])
                vip = [torch.argmax(unlabeled_logits[:,1]).item()]
                vipsIdx = [i for i in range(vipMask.shape[0]) if vipMask[i] == 1]
                indices = list(range(unlabeled_logits.shape[0]))
                nonvipIdx = list(set(indices).difference(set(vipsIdx)))

                select_unsrc1,select_unface1,select_unfaceCont1,select_uncoor1,select_unlabel=[],[],[],[],[]
                ...
                unlabel = torch.ones(args.ni,)
                unlabel[0] = 1

                unlabel_srcs.append(unsrc1)
                unlabel_faces.append(unface1)
                unlabel_faceconts.append(unfaceCont1)
                unlabel_coors.append(uncoor1)
                unlabel_labels.append(unlabel)

        unlabeled_Ifeat = []
        for src,face,facecont,coor,label in zip(unlabel_srcs, unlabel_faces,unlabel_faceconts,unlabel_coors,unlabel_labels):
            unlabeled_gfeat,unlabeled_rfeat = Encoder(src,face,facecont,coor,label)
            unIfeat = Relation(unlabeled_gfeat,unlabeled_rfeat)
            unlabeled_Ifeat.append(unIfeat)
        unlabeled_Ifeat = torch.cat(unlabeled_Ifeat)
        unlabeled_logits = Classifier(unlabeled_Ifeat)
        unlabel_labels = torch.cat(unlabel_labels,0).cuda().long()
        celoss2 = CEloss(unlabeled_logits,unlabel_labels)
